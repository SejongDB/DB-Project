...
private static void showReservationForm(JFrame userFrame) {
    ...
    reservationPanel.setPreferredSize(new Dimension(400, 150));
    ...
    reservationPanel.add(scheduleIdLabel);
    reservationPanel.add(scheduleIdText);
    reservationPanel.add(seatIdLabel);
    reservationPanel.add(seatIdComboBox);
    ...
    reservationPanel.add(new JLabel()); // 빈 레이블로 레이아웃 채우기
        ...
        while (hasResults) {
            ...
            table.addMouseListener(new MouseAdapter() {
                public void mouseClicked(MouseEvent e) {
                    ...
                    if (row != -1) {
                        String ticketIdStr = table.getValueAt(row, 5).toString(); // Assuming ticket_id is the 6th column (index 5)
                        try {
                            int ticketId = Integer.parseInt(ticketIdStr); // Convert the ticket_id to Integer
                            showReservationDetails(ticketId);
                        } catch (NumberFormatException ex) {
                            JOptionPane.showMessageDialog(userFrame, "Invalid ticket ID format.", "Error", JOptionPane.ERROR_MESSAGE);
                        }
                    }
                }
            });
...
private static void showReservationDetails(int ticketId) {
    ...
    detailsPanel.setPreferredSize(new Dimension(500, 300));
